...
win_x11* win_x11_init(stack_alloc* alloc) {
    ...
    if (!win->display) {
        print_string(file_stderr(), STR_SPAN("win_x11: Cannot open display\n"));
        sa_free(alloc, win);
        return alloc->cursor;
    }
...
i32 win_x11_open_window(win_x11* win, const char* title, i32 width, i32 height, stack_alloc* alloc) {
    ...
    if (!win->window) {
        print_string(file_stderr(), STR_SPAN("win_x11: Cannot create window\n"));
        return 0;  // false
    }
    ...
    if (!win->pixel_buffer_img) {
        print_string(file_stderr(), STR_SPAN("win_x11: Cannot create XImage\n"));
