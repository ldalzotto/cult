...
i32 main() {
    ...
    if (win_ctx == NULL) {
        print_string(file_get_stderr(), "Failed to initialize X11 window context\n");
    ...
    if (window_created == 0) {
        print_string(file_get_stderr(), "Failed to create window\n");
        win_x11_deinit(&win_alloc, win_ctx);
        mem_unmap(win_mem, win_mem_size);
        return 1;
    }

    print_string(file_get_stdout(), "Window created successfully. Press Ctrl+C to exit.\n");
    ...
    while (1) {
        ...
        for (win_event* event = event_begin; event < event_end;++event) {
            ...
            print_format(file_get_stdout(), "Received event type: %d\n", event->type);
            if (event->type == 2) {
                should_exit = 1;
            }
        }
