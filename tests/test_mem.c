...
void test_mem_map_small_block() {
    ...
    if (p != MAP_FAILED) {
        *(u8*)p = (u8)'X';
        TEST_ASSERT_EQUAL(*(u8*)p, (u8)'X');
    }

    mem_unmap(p, size);
}

void test_mem_map_zero_size() {
    uptr size = 0;
    void* p = mem_map(size);
    TEST_ASSERT_NOT_NULL(p);
    TEST_ASSERT_TRUE(p != MAP_FAILED);
...
void test_mem_map_large_block() {
    ...
    if (p != MAP_FAILED) {
        ...
        *(u8*)p = (u8)'A';
        TEST_ASSERT_EQUAL(*(u8*)p, (u8)'A');

        *(u8*)byteoffset(p, size - 1) = (u8)'Z';
        TEST_ASSERT_EQUAL(*(u8*)byteoffset(p, size - 1), (u8)'Z');
    }
...
void test_mem_module() {
    ...
    test_mem_map_small_block();
    test_mem_map_zero_size();
    test_mem_map_large_block();
}
